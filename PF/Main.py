...
def Main():
    ...
    while opcion:
        ...
        if opcion2=="1" or opcion2=="REGISTRARSE":
            ...
            rfc=input("\t📜 Ingrese su RFC: ").upper().strip()
            Validacion=Funciones.ValidacionRFC(rfc)
            while Validacion==False:
               rfc=input("\t⚠️ Por favor ingrese un rfc valido: ").lower().strip()
               Validacion=Funciones.ValidacionRFC(rfc)
            ...
            respuesta=empleado.AgregarEmpleado(Nombre,Apellidos,rfc,Password)
            if respuesta=="Correct":
                print(f"\n\t::: ✅ Se ha registrado {Nombre} {Apellidos} con exito :::")
            elif respuesta=="key duplicada":
                print("\n\t ⚠️  El rfc ingresado ya existe en el sistema")
            elif respuesta==None:
            ...
            rfc=input("\t📜 Ingrese su RFC de 13 caracteres: ").upper().strip()
            Validacion=Funciones.ValidacionRFC(rfc)
            while Validacion==False:
                rfc=input("\t⚠️ Por favor ingrese un rfc valido: ").lower().strip()
                Validacion=Funciones.ValidacionRFC(rfc)
            ...
            while Password=="" or (len(Password)<10 or len(Password)>14):
               Password=getpass.getpass("\t[::] Debe ingresar una contraseña con al menos 10  y maximo 14 caracteres : ").strip()
            lista_empleados=empleado.Inicio_sesion(rfc,Password)
            try:
               ...
               print("\n\t⚠️  RFC y/o contraseña incorrectas.Por favor intente de nuevo")
